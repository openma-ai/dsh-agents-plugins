...
export class CodexRelayMcpProtocol {
  ...
  constructor(private readonly relay: CodexHostRelay) {}

  async handle(message: McpRequest): Promise<Record<string, unknown> | undefined> {
  ...
  private async dispatch(method: string, params: unknown): Promise<unknown> {
    ...
    if (!this.initialized) throw new Error('MCP relay is not initialized')
    if (method === 'ping') return {}
    if (method === 'tools/list') return { tools: this.relay.listTools() }
    if (method === 'tools/call') {
      const input = record(params, 'tools/call params')
      const name = typeof input.name === 'string' ? input.name : undefined
      if (name === undefined) throw new Error('tools/call params.name must be a string')
    ...
    if (method === 'resources/list') return { resources: this.relay.listResources() }
    ...
    if (method === 'resources/read') {
      ...
      if (uri === undefined) throw new Error('resources/read params.uri must be a string')
...
async function main(): Promise<void> {
  ...
  input.on('line', line => {
    chain = chain.then(async () => {
      ...
      const message = record(parsed, 'MCP request') as unknown as McpRequest
      if (typeof message.method !== 'string') throw new Error('MCP request method must be a string')
  ...
  await new Promise<void>(resolveDone => { input.once('close', () => resolveDone()) })
  await chain
  await relay.close()
}
