...
const RELEASE_DIRECTORIES = ['packages/theme-adapter', 'packages/ui', '.'] as const

/** Build the dependency-safe workspace publication order for one repository tag. */
export async function createReleasePlan(root: string, tag: string): Promise<ReleasePackage[]> {
