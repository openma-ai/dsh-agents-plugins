...
function frontmatter(markdown: string): Frontmatter {
  ...
  if (end < 0) return { attributes: {}, body: markdown.trim() }
  const attributes: Record<string, string> = {}
  for (const line of normalized.slice(4, end).split('\n')) {
    const match = /^([A-Za-z0-9_-]+):\s*(.*)$/u.exec(line)
    if (match?.[1] !== undefined && match[2] !== undefined) attributes[match[1]] = scalar(match[2])
  }
  return { attributes, body: normalized.slice(end + 5).trim() }
}

function markdownFiles(root: string, target: string, recursive: boolean): string[] {
  const realRoot = realpathSync(root)
  const realTarget = realpathSync(target)
  if (!contained(realRoot, realTarget)) throw new TypeError(`prompt component resolves outside plugin root: ${target}`)
  if (statSync(realTarget).isFile()) return extname(realTarget).toLowerCase() === '.md' ? [realTarget] : []
  if (!statSync(realTarget).isDirectory()) return []
  const files: string[] = []
  for (const entry of readdirSync(realTarget, { withFileTypes: true })) {
    ...
    if (!contained(realRoot, real)) continue
    if (entry.isDirectory() && recursive) files.push(...markdownFiles(realRoot, real, true))
...
function piMarkdownFiles(root: string, entries: readonly string[]): string[] {
  ...
  for (const expression of entries.filter(entry => !entry.startsWith('!'))) {
    ...
    if (!contained(root, target)) throw new TypeError(`Pi prompt path "${expression}" escapes the plugin root`)
...
function commandSuffix(componentRoot: string, file: string, declaredName?: string): string {
  if (declaredName !== undefined && declaredName.trim().length > 0) return slug(declaredName)
...
function projectDir(agent: Agent): string {
  const cwd = agent.session.header.cwd
  return typeof cwd === 'string' && cwd.length > 0 ? cwd : process.cwd()
}

export function expandPrompt(body: string, rawInput: string, config: Config, agent: Agent): string {
  ...
  return body
    ...
    .replace(/\$\{(?:@|ARGUMENTS):-([^}]*)\}/gu, (_match, fallback: string) => all || fallback)
    .replace(/\$\{(\d+):-([^}]*)\}/gu, (_match, index: string, fallback: string) => args[Number(index) - 1] || fallback)
    .replace(/\$\{@:(\d+)(?::(\d+))?\}/gu, (_match, start: string, length?: string) => {
    ...
    .replace(/\$(\d+)/gu, (_match, index: string) => args[Number(index) - 1] ?? '')
}

function templates(config: Config): PromptTemplate[] {
  ...
  if (!contained(pluginRoot, componentRoot)) throw new TypeError('prompt component escapes plugin root')
