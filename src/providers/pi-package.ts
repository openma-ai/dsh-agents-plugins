...
function packageManifest(value: unknown): Record<string, unknown> {
  if (!record(value)) throw new TypeError(`${FORMAT}: package.json must contain an object`)
...
function normalizeResourcePath(value: string, field: string): string {
  if (value.length === 0) {
...
): { readonly components: readonly PackageComponent[]; readonly diagnostics: readonly string[] } {
...
function conventionalResources(source: PluginPackageSource): {
  ...
  const diagnostics: string[] = []
  for (const resource of RESOURCES) {
    if (!source.has(resource.convention)) continue
...
export const piPackageProvider: PackageFormatProvider = {
  name: FORMAT,
  probe(source: PluginPackageSource): PackageFormatObservation | undefined {
    if (!source.has(MANIFEST_PATH) || source.kind(MANIFEST_PATH) !== 'file') return undefined
    const manifest = packageManifest(source.readJson(MANIFEST_PATH))
    if (Object.hasOwn(manifest, 'pi')) {
      if (!record(manifest.pi)) throw new TypeError(`${FORMAT}: package.json field "pi" must be an object`)
    ...
    if (!observation.claimed) return undefined
    return {
      manifestPath: MANIFEST_PATH,
      manifest: Object.freeze({ ...manifest }),
      components: observation.components,
