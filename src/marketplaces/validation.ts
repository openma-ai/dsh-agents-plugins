...
export function requirePlugins(manifest: Record<string, unknown>): readonly unknown[] {
...
export function parseMarketplaceRelativeDirectory(
  value: unknown,
  field: string,
): MarketplaceRelativeDirectorySource {
...
export function parseGitHubRepositorySource(
  value: Record<string, unknown>,
  field: string,
): GitHubRepositorySource {
  const unsupportedKey = Object.keys(value).find(
    key => key !== 'source' && key !== 'repo' && key !== 'ref' && key !== 'sha',
  )
  ...
  const result: {
    ...
    ref?: string
    sha?: string
  } = { kind: 'github-repository', repo: value.repo }
  if (value.ref !== undefined) result.ref = requireNonEmptyString(value.ref, `${field}.ref`)
  if (value.sha !== undefined) result.sha = requireNonEmptyString(value.sha, `${field}.sha`)
  return result
}
...
export function parsePluginEntry(
  value: unknown,
  index: number,
  parseSource: (value: unknown, field: string) => MarketplacePluginEntry['source'],
): MarketplacePluginEntry {
...
export function requireUniquePluginNames(entries: readonly MarketplacePluginEntry[]): void {
  const names = new Set<string>()
  for (const entry of entries) {
    if (names.has(entry.name)) throw new TypeError(`duplicate plugin name "${entry.name}"`)
    names.add(entry.name)
  }
}

function isSafeMarketplaceRelativePath(value: string): boolean {
