...
export async function apply(ctx: Context, rawConfig: Config): Promise<void> {
  ...
  let catalog: CodexAppApprovalCatalog = createCodexAppApprovalCatalog(config.serverName, [])
  let relay: CodexHostRelay | undefined
  try {
    ...
    ctx.logger.warn(
      `codex-app-tool-approval(${config.appName}): could not load read-only catalog; all relay tools require approval: ${String(error)}`,
    )
  } finally {
    await relay?.close()
  }

  ctx.on('tools/pre-execute', (execution, next): Promise<PreToolDecision> => {
    if (!catalog.requiresApproval(execution.name)) return next()
