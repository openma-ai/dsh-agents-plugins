...
function expand(value, pluginRoot, pluginData) {
    return value
        ...
        for (const serverName of Object.keys(servers).sort()) {
            try {
                ...
                if (server.workspaceFolder !== undefined) {
                    if (typeof server.workspaceFolder !== 'string')
                        throw new TypeError(`${label}.workspaceFolder must be a string`);
                ...
                if (server.initializationOptions !== undefined)
                    config.initializationOptions = server.initializationOptions;
                if (server.configuration !== undefined)
                    config.configuration = server.configuration;
                if (server.shutdownTimeout !== undefined) {
                    if (typeof server.shutdownTimeout !== 'number')
                        throw new TypeError(`${label}.shutdownTimeout must be a number`);
...
//# sourceMappingURL=dsh-lsp.js.map