...
function skillFiles(root, resource) {
...
function resolveSkillFiles(config) {
    ...
    for (const expression of include) {
        ...
        if (!hasPattern) {
            ...
            if (!contained(root, target))
                throw new TypeError(`Pi skill path "${expression}" escapes the plugin root`);
            let real;
            try {
        ...
        if (!contained(root, start))
            throw new TypeError(`Pi skill pattern "${expression}" escapes the plugin root`);
        for (const path of walk(root, start)) {
            const local = posix(relative(root, path));
            if (minimatch(local, expression, { dot: true }))
                resources.add(path);
        }
    }
    const files = new Set();
    for (const resource of resources) {
        for (const file of skillFiles(root, resource)) {
            ...
            if (exclude.some(pattern => minimatch(localFile, pattern, { dot: true })
                || minimatch(localBundle, pattern, { dot: true })))
                continue;
            files.add(file);
        }
    }
...
//# sourceMappingURL=pi-skills.js.map