...
function discoveryGroup(locator) {
    if (locator.startsWith('claude-code-'))
        return 'Claude Code';
    if (locator.startsWith('codex-'))
        return 'Codex';
    if (locator.startsWith('pi-'))
        return 'Pi';
    return locator;
}
function discoverySummary(candidate) {
    const details = [
        ...
        return runtime.discoverLocalPlugins().then(result => {
            ...
            for (const [group, candidates] of groups) {
                ...
                for (const candidate of candidates) {
                    lines.push(discoverySummary(candidate), `  - Import: /plugin-bridge import ${candidate.ref}`);
                }
            }
