...
function withDiagnostics(lines, diagnostics) {
    const output = [...lines];
    if (diagnostics.length > 0)
        output.push('Diagnostics:', ...diagnostics.map(item => `- ${item}`));
    return output.join('\n');
}
function installationText(action, plugin) {
...
export function executePluginBridgeCommand(kernel, rawInput, runtime) {
    ...
    if (importPlugin?.[1] !== undefined) {
        ...
        return runtime.importLocalPlugin(importPlugin[1]).then(plugin => ({
            kind: 'success',
            text: installationText(`Imported ${plugin.name} from ${plugin.marketplace.replace(/^import:/u, '')}`, plugin),
        }));
    }
    const reviewHooks = /^hooks review(?: (\S+))?$/u.exec(input);
    if (reviewHooks !== null) {
        if (runtime === undefined)
            return unavailable();
        return runtime.reviewActivations('hook-user-approval', reviewHooks[1]).then(reviews => {
            if (reviews.length === 0)
                return { kind: 'success', text: 'No bundled hooks require review.' };
            return {
                kind: 'success',
                text: reviews.map(review => [
                    `[${review.plugin}] ${review.approved ? 'approved' : 'pending'} ${review.rowId}`,
                    review.review,
                ].join('\n')).join('\n\n'),
            };
        });
    }
    const approveHooks = /^hooks approve (\S+) ([a-fA-F0-9]{64})$/u.exec(input);
    if (approveHooks?.[1] !== undefined && approveHooks[2] !== undefined) {
        if (runtime === undefined)
            return unavailable();
        const plugin = approveHooks[1];
        const digest = approveHooks[2].toLowerCase();
        return runtime.approveActivation('hook-user-approval', plugin, digest).then(() => ({
            kind: 'success',
            text: `Approved ${plugin} hooks at digest ${digest}.`,
        }));
    }
...
export function apply(ctx) {
    ctx.commands.register({
        ...
        input: { hint: '[marketplace|install|discover|import|hooks|enable|disable|uninstall] ...' },
        handler: invocation => executePluginBridgeCommand(ctx.pluginBridge, invocation.rawInput, ctx.pluginBridgeRuntime),
    });
}
