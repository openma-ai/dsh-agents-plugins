...
function directFiles(root, target) {
    let real;
    try {
...
export function resolvePiExtensionFiles(pluginRoot, entries) {
    ...
    for (const expression of entries.filter(entry => !entry.startsWith('!'))) {
        ...
        if (!contained(root, start))
            throw new TypeError(`Pi extension path "${expression}" escapes the plugin root`);
...
//# sourceMappingURL=pi-extension-files.js.map