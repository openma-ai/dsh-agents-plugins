...
function walk(root, start) {
    ...
    for (const entry of readdirSync(realStart, { withFileTypes: true })) {
        ...
        if (entry.isDirectory())
            values.push(...walk(root, real));
        else if (entry.isFile())
            values.push(real);
    }
    return values;
}
function themeFiles(pluginRoot, entries) {
    ...
    for (const expression of entries.filter(entry => !entry.startsWith('!'))) {
        ...
        if (!contained(root, start))
            throw new TypeError(`theme path "${expression}" escapes the plugin root`);
...
function color(value, variables, seen = new Set()) {
    ...
    if (!Object.hasOwn(variables, value) || seen.has(value))
        return value;
    seen.add(value);
    return color(variables[value], variables, seen);
}
function mappedTokens(values, variables, mapping) {
    const tokens = {};
...
function piTheme(value, pluginName, file) {
...
function claudeTheme(value, pluginName, file) {
...
export function loadThemeDefinitions(dialect, pluginName, pluginRoot, entries) {
    const themes = [];
    const diagnostics = [];
    const seen = new Set();
    for (const file of themeFiles(pluginRoot, entries)) {
        try {
            ...
            if (seen.has(theme.id))
                throw new TypeError(`${file}: duplicate mapped theme id "${theme.id}"`);
            seen.add(theme.id);
            themes.push(theme);
        }
        catch (error) {
...
//# sourceMappingURL=theme-definitions.js.map