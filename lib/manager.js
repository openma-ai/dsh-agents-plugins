...
async function updateDirectoryDigest(root, current, hash) {
    for (const entry of await readdir(current, { withFileTypes: true }).then(entries => (entries.filter(item => item.name !== '.git').sort((left, right) => compareCodePoints(left.name, right.name))))) {
        const path = join(current, entry.name);
        const relativePath = path.slice(root.length + 1);
        hash.update(entry.isDirectory() ? `d\0${relativePath}\0` : `f\0${relativePath}\0`);
        if (entry.isDirectory())
            await updateDirectoryDigest(root, path, hash);
        else
...
export class PluginBridgeManager {
    ...
    async syncImportedPlugins() {
        ...
        for (let index = 0; index < this.state.installations.length; index += 1) {
            ...
            if (current.marketplace !== `import:${locator}`)
                continue;
            let candidates = discovery.candidates.filter(candidate => (candidate.locator === locator
            ...
            if (locator === 'codex-local-cache') {
                ...
                candidate = candidates.sort((left, right) => (comparePluginVersions(left.version, right.version))).at(-1);
            }
            ...
            try {
                ...
                if (!followsInstalledRegistry
                    ...
                    && comparePluginVersions(candidate.version, current.source.version) < 0)
                    continue;
                ...
                diagnostics.push(`${current.name}: auto-update to ${candidate.version} failed: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
    ...
    async addMarketplace(location, preferredManifestPath) {
        await this.ensureStarted();
        const github = githubLocation(location);
        let remoteStage;
        let target;
        let root;
        let requestedPath = preferredManifestPath;
        if (github !== undefined) {
    ...
    importedPluginDataRoot(installation, candidate) {
        if (installation.source?.dataRoot !== undefined)
            return installation.source.dataRoot;
        for (const row of installation.rows) {
            const pluginData = row.config?.env?.PLUGIN_DATA;
    ...
    async replaceImportedPlugin(index, candidate, digest) {
        const current = this.state.installations[index];
        if (current === undefined)
            throw new Error('cannot update a missing installation');
        const previousApprovals = structuredClone(this.state.approvals);
        const dataRoot = this.importedPluginDataRoot(current, candidate);
        ...
        try {
            ...
            if (current.enabled) {
                for (const row of [...current.rows].reverse()) {
                    if (!this.activeRowIds.includes(row.id))
                        continue;
                    await this.loader.remove(row.id);
                    removed.push(row);
                    const activeIndex = this.activeRowIds.indexOf(row.id);
            ...
            const validApprovals = new Set(nextInstallation.activations.map(requirement => (`${requirement.policy}\0${requirement.rowId}\0${requirement.digest}`)));
            const nextApprovals = previousApprovals.filter(approval => (!ownedRows.has(approval.rowId)
                || validApprovals.has(`${approval.policy}\0${approval.rowId}\0${approval.digest}`)));
            if (nextInstallation.enabled) {
                for (const row of this.rowsAllowedByPolicyWithApprovals(nextInstallation.rows, nextInstallation.activations, nextApprovals)) {
                    try {
                        await this.createRow(row);
                        created.push(row.id);
                    }
            ...
            this.activeRowIds.push(...created);
            try {
                await this.moveToTrash(backup);
            }
            catch { /* the active replacement is already durable */ }
        }
        catch (error) {
            this.state.installations[index] = current;
            this.state.approvals.splice(0, this.state.approvals.length, ...previousApprovals);
            await this.removeRows(created);
            if (newPromoted)
                await this.moveToTrash(current.root);
            else
                await rm(stage, { recursive: true, force: true });
            if (oldMoved)
                await rename(backup, current.root);
