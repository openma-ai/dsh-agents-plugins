...
function npmSource(value, installRoot) {
...
function parseSource(value, settingsPath) {
...
function sourceEntry(value, index) {
...
function candidateKey(scope, source, canonicalRoot) {
...
export function createPiInstalledPluginLocator(config = {}) {
            ...
            for (const [index, unknownEntry] of settings.packages.entries()) {
                ...
                try {
                    ...
                    candidates.push({
                        ...
                        name: observation.manifest.name,
                        root,
                    ...
                    diagnostics.push(`pi-installed: packages[${index}] "${entry}" is invalid: ${error instanceof Error ? error.message : String(error)}`);
                }
            }
            return {
                candidates,
