...
function validateOptionalString(manifest, field) {
...
function discoverMcp(source) {
    if (!source.has('mcp.json'))
...
export const agentPluginsV1Provider = {
    name: 'agent-plugins-v1',
    probe(source) {
        if (!source.has('plugin.json'))
            return undefined;
        const candidate = source.readJson('plugin.json');
        if (!record(candidate) || candidate.$schema !== AGENT_PLUGINS_V1_SCHEMA)
            return undefined;
...
//# sourceMappingURL=agent-plugins-v1.js.map