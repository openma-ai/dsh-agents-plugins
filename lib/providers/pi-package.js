...
function declaredResources(source, pi) {
    ...
    for (const resource of RESOURCES) {
        ...
        const normalizedEntries = declared.map(entry => normalizeResourcePath(entry, resource.field));
        for (const [index, path] of normalizedEntries.entries()) {
            if (isPattern(path))
                continue;
            if (!source.has(path)) {
                diagnostics.push(`${FORMAT}: pi.${resource.field} path "${declared[index]}" does not exist; resource disabled`);
            }
        }
