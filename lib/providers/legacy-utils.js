...
export function manifestObject(value, format) {
...
export function normalizePluginPath(value, format, field) {
...
export function declaredComponents(source, manifestPath, manifest, field, type, format) {
    ...
    for (const entry of values(declared)) {
        if (typeof entry === 'string') {
            ...
            if (!source.has(path))
                throw new TypeError(`${format}: ${field} path "${entry}" does not exist`);
...
export function fixedComponent(source, type, path) {
