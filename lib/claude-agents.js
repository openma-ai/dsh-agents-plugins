...
function markdownFiles(root, target) {
    const realTarget = realpathSync(target);
    if (!contained(root, realTarget))
        throw new TypeError(`Claude agents component resolves outside plugin root: ${target}`);
    ...
    for (const entry of readdirSync(realTarget, { withFileTypes: true })) {
        ...
        if (!contained(root, real))
            throw new TypeError(`Claude agents entry resolves outside plugin root: ${path}`);
        if (entry.isDirectory())
            files.push(...markdownFiles(root, real));
...
function frontmatter(markdown) {
    ...
    if (end < 0)
        throw new TypeError('unterminated YAML frontmatter');
...
function requiredString(attributes, field) {
...
function declaredTools(value) {
    ...
    if (typeof value === 'string') {
        ...
        if (names.length === 0)
            throw new TypeError('tools must name at least one Claude tool');
        return names;
    }
...
function inspectFile(config, root, file, diagnostics) {
    ...
    try {
        ...
        if (parsed.body.length === 0)
            throw new TypeError('agent body must be non-empty');
    }
    catch (error) {
        diagnostics.push(`${label} is not importable: ${error instanceof Error ? error.message : String(error)}`);
        return undefined;
    ...
    if (model !== undefined && model !== 'inherit') {
        diagnostics.push(`${label} declares Claude model ${JSON.stringify(model)}, which has no exact DSH per-child model mapping; `
    ...
    for (const claudeTool of tools ?? []) {
        ...
        if (dshTool === undefined) {
            diagnostics.push(`${label} declares Claude tool ${JSON.stringify(claudeTool)}, which has no exact DSH tool mapping and was omitted from the child allow-list`);
            continue;
        }
        if (!allow.includes(dshTool))
            allow.push(dshTool);
    }
    return {
        source,
...
export function inspectClaudeAgents(config) {
    const root = realpathSync(config.pluginRoot);
    const component = resolve(root, config.componentPath);
    if (!contained(root, component))
        throw new TypeError('Claude agents component escapes plugin root');
    const diagnostics = [];
    const agents = [];
    const names = new Set();
...
function abnormalResult(result) {
...
export function apply(ctx, config) {
    ...
    let toolDisposers = [];
    const unmount = () => {
        for (const dispose of toolDisposers.splice(0).reverse())
            dispose();
    };
                ...
                mounted.push(ctx.tools.register(defineTool({
                    ...
                    async execute(args, exec) {
                        if (exec.agent === undefined)
                            throw new Error('Claude agent tool requires a calling DSH agent');
    ...
    ctx.on('subagent/provider-added', (provider) => {
        if (provider.name === config.provider && toolDisposers.length === 0)
            mount(provider);
    });
